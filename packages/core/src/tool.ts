...
export function defineTool<
  TParams extends ZodSchema,
  TReturns extends ZodSchema | undefined = undefined,
>(definition: ToolDefinition<TParams, TReturns>): Tool<TParams, TReturns> {
  if (!definition.name || typeof definition.name !== "string") {
    throw new Error("Tool name is required");
  }
  if (!/^[a-zA-Z][a-zA-Z0-9_]*$/.test(definition.name)) {
    throw new Error(
      `Invalid tool name "${definition.name}": must start with a letter and contain only alphanumeric characters and underscores`
    );
  }
  ...
  if (getZodType(definition.parameters) !== "object") {
    throw new Error(
      `Tool parameters must be z.object(), got z.${getZodType(definition.parameters)}()`
    );
  }
 ...
 */
// biome-ignore lint/complexity/noBannedTypes: Function is intentional for generic function serialization
